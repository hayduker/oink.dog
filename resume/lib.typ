...
#let generic_3x2(cols, r1c1, r1c2, r2c1, r2c2, r3c1, r3c2) = {
  assert.eq(type(cols), array)

  grid(
    columns: cols,
    align(left)[#r1c1 \ #r2c1 \ #r3c1],
    align(right)[#r1c2 \ #r2c2 \ #r3c2]
  )
}
...
#let double-education-heading(institution, location, degree0, major0, start-date0, end-date0, degree1, major1, start-date1, end-date1, body) = {
  assert.eq(type(start-date0), datetime)
  assert(type(end-date0) == datetime or type(end-date0) == str)

  assert.eq(type(start-date1), datetime)
  assert(type(end-date1) == datetime or type(end-date1) == str)

  generic_3x2(
    (70%, 30%),
    [*#institution*], [*#location*], 
    [#degree0, #major0], period_worked(start-date0, end-date0),
    [#degree1, #major1], period_worked(start-date1, end-date1)
  )
  v(-0.2em)
  if body != [] {
    v(-0.4em)
